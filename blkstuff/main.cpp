...
void create_decrypt_vector(uint8_t* key, uint8_t* encrypted_data, uint64_t encrypted_size, uint8_t* output, uint64_t output_size) {
    if (output_size != 4096) {
        spdlog::error("create_decrypt_vector does not support an output_size other than 4096");
        exit(1);
    }
    ...
    for (size_t i = 0; i < encrypted_size >> 3; i++) {
...
void mhy0_header_scramble(uint8_t* input, uint64_t limit, uint8_t* input2, uint64_t chunk_size) {
    if (!((limit == 0x39 && chunk_size == 0x1C) || (limit == 0x21 && chunk_size == 8))) {
        spdlog::error("unsupported parameters for mhy0_header_scramble");
        exit(1);
    }
...
void mhy0_extract(const char* out_format, int block_index, uint8_t* input, size_t input_size) {
    ...
    if (*(uint32_t*)input != 0x3079686D) { // mhy0
        spdlog::error("decrypted data didn't start with mhy0, so decryption probably failed");
        exit(1);
    }
    ...
    if (size > input_size) {
        ...
        spdlog::error("attempted to get 0x{:x} bytes out of a 0x{:x} input! skipping mhy0 {:d}", size, input_size, block_index);
        return;
    }
    ...
    if (lz4_res < 0) {
        ...
        spdlog::error("decompression failed: {}", lz4_res);
        exit(1);
    }
    delete[] data;
...
int extract_blk(const char* in_filename, const char* out_format) {
    auto* blk_file = fopen(in_filename, "rb");

    if (!blk_file) {
        spdlog::critical("failed to open blk. file: {}", in_filename);
        return 1;
    }
    ...
    if (fread(&hdr, sizeof(blk_header), 1, blk_file) < 1) {
        spdlog::critical("Failed to read BLK header! file: {}", in_filename);
        fail = true;
    }

    if (!fail && hdr.magic != 0x6B6C62) { // blk\x00
        spdlog::critical("bad file magic. file: {}", in_filename);
        fail = true;
    }

    if (!fail && hdr.version != 0x10) {
        spdlog::critical("version is not 0x10. file: {}", in_filename);
        fail = true;
    }
...
void print_help() {
    ...
    cout << "usage:" << endl
        << "blkstuff [[-s <path>] | [-d <file>]] [-o <path=./output>] [-t <thread_count=16>] [-l <file>] [-h]" << endl
        << " -s, --scan FOLDER         scan all the blk files in FOLDER and decrypt them" << endl
        << " -d, --decrypt BLK_FILE    decrypt BLK_FILE" << endl
        << " -o, --output FOLDER       write unlocked blks to FOLDER, default: ./output" << endl
        << " -t, --thread_count COUNT  specify the thread count to decrypt blk files, default: 16" << endl
        << " -l, --log_file FILE       set the log file to save the log. log will NOT be saved if this option is not specified" << endl
        << " -h, --help                print this message" << endl
        << endl
        << "notice:" << endl
        << " if you specify -s and -d in the same time the -d option will be ignore" << endl;
}
...
int main(int argc, char** argv) {
    ...
    if (cmdl({ "l", "log_file" })) {
        try {
            auto file_rotating_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(cmdl({ "l", "log_file" }).str(), 1024 * 1024 * 10, 3);
            spdlog::default_logger()->sinks().push_back(file_rotating_sink);
    ...
    std::string output_folder = cmdl({ "o", "output" }, "./output").str();
    auto output_base = std::filesystem::path(output_folder);

    spdlog::info("output path: {}", output_base.generic_string());

    if (cmdl({ "s", "scan" })) {
        std::string scan_folder = cmdl({ "s", "scan" }).str();

        auto base_path = std::filesystem::path(scan_folder);
        std::vector<std::filesystem::path> blk_paths;
        spdlog::info("scanning for blks in {}", scan_folder);
        try {
            ...
            spdlog::error("failed to search for blk files with error: {}", e.what());
            return 1;
        }
        spdlog::info("found {} blks to extract", blk_paths.size());

        size_t max_thread_count;
        cmdl({ "t", "thread_count" }, 16) >> max_thread_count;

        clock_t c = clock();
        {
            ThreadPool extract_blk_pool(max_thread_count);
            for (size_t i = 0; i < blk_paths.size(); i++) {
                extract_blk_pool.enqueue([&, i] {
                        auto p = blk_paths[i];
                        spdlog::info("processing {}...", p.generic_string());
                        ...
                        if (ret) {
                        spdlog::warn("failed to extract {}", input_path.generic_string());
                        } else {
                        spdlog::info("successfully extract {}", input_path.generic_string());
                        }
                        }
                        );
            }
        }
        spdlog::info("time: {}ms", clock() - c);
    } else if (cmdl({ "d", "decrypt" })) {
        std::string blk_path = cmdl({ "d", "dectypt" }).str();
        spdlog::info("processing {}...", blk_path);
        auto blk_name = std::filesystem::path(blk_path).filename();
        auto output_path = (output_base / blk_name).replace_extension(".%d.bin");
        auto output_dir = std::filesystem::path(output_path).remove_filename();
        std::filesystem::create_directories(output_dir);
        int ret = extract_blk(blk_path.c_str(), output_path.generic_string().c_str());
        if (ret) {
            spdlog::warn("failed to extract {}", blk_path);
        } else {
            spdlog::info("successfully extract {}", blk_path);
        }
    } else {
        spdlog::error("wrong argument");
        print_help();
    }
}
